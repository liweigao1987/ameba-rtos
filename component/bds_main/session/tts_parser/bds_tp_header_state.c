...
static void decode(bds_tts_parser_t *machine, bdsc_event_data_t *data, int offset) {
    ...
    int header_lack = tts_frame_header_lack(machine->frame);
    if (header_lack > 0) {
        if (left >= header_lack) {
            tts_frame_clone_header(machine->frame, start_addr, header_lack);
            offset += header_lack;
            bds_set_tts_parser_state(machine, &machine->audio_length_state->base_state);
            bds_tts_parser_decode(machine, data, offset);
        } else {
            tts_frame_clone_header(machine->frame, start_addr, left);
            offset += left;
        }
    } else {
        bds_set_tts_parser_state(machine, &machine->audio_length_state->base_state);
...
bds_tp_header_state_t* bds_tp_header_state_create() {
    bds_tp_header_state_t *state = bdsc_malloc(sizeof(bds_tp_header_state_t));
